...
学科培训课程编写与管理工具。它面向课程生产流程，支持从 Bilibili 视频资源获取字幕与封面，调用大模型生成课程素材，再在可视化课程编辑器中整理 `course.json`，最后对接学科培训后端完成课程上传、测试、导出和删除。

当前项目的主线目标是：让课程制作人员可以在一个本地工具内完成“视频处理 -> 课程编写 -> 课程库管理”的完整闭环。

## 当前能力

- Web 可视化界面，默认运行在 `http://127.0.0.1:7200`
- Bilibili 视频、分 P 视频、收藏夹批量处理
- 在线字幕读取，缺少在线字幕时支持本地 ASR 转录
- 支持 SESSDATA 配置，用于读取需要登录状态的视频字幕
- 下载视频封面、字幕和视频基础信息
- 调用大模型生成：
  - 知识要点 `summary.json`
  - 知识正文 `.md`
...
- 可视化课程结构编辑：
  ...
  - 章 / 小节结构
  - 小节知识正文
  - 知识要点 `knowledge_points.key_points`
...
- 课程 JSON 规范化：
  - 自动补齐章、小节主键
  - 兼容旧格式中的 `id`
  - 将旧格式空数组 `knowledge_points: []` 规范化为 `{ "key_points": [] }`
- 学科培训课程库管理：
  - 连接后端，默认 `http://127.0.0.1:7100`
  - 上传当前课程
  - 测试读取当前课程或线上课程
  - 导出线上课程 JSON
  - 删除线上课程
- 面向启动器的一次性动态构建方案，详见 [doc/launcher-dynamic-build.md](doc/launcher-dynamic-build.md)
...
- [普通用户使用指南](temp/共工计划-学科培训课程编写与管理工具使用指南.md)
